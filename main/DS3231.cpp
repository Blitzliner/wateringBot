...
#define TEMPERATURE_OFFSET -2.6
DS3231::DS3231() {}

/***************************************** 
	Public Functions
 *****************************************/
byte DS3231::_readByte(uint8_t address, uint8_t regist) {
...
void DS3231::_writeByte(uint8_t address, uint8_t regist, uint8_t msg) {
  Wire.beginTransmission(address);
  Wire.write(regist);
  Wire.write(msg);
  Wire.endTransmission();
...
void DS3231::getTime(byte& year, byte& month, byte& date, byte& DoW, byte& hour, byte& minute, byte& second) {
	...
	tempBuffer = Wire.read(); /* TODO: has to be removed */
	(void)bcdToDec(Wire.read());
	...
	year = _decodeY(Wire.read());// + 2000;//bcdToDec(Wire.read());// + 30;
}
...
byte DS3231::getYear() {
  return (_decodeY(_readByte(CLOCK_ADDRESS, 0x06)));// + 2000);
}
...
float DS3231::getTemperature() {
	...
	return (float(temp) + 0.25*(Wire.read()>>6)) + TEMPERATURE_OFFSET;
}
